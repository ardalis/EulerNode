...
const sourceData = `08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08
49 49 99 40 17 81 18 57 60 87 17 40 98 43 69 48 04 56 62 00
81 49 31 73 55 79 14 29 93 71 40 67 53 88 30 03 49 13 36 65
52 70 95 23 04 60 11 42 69 24 68 56 01 32 56 71 37 02 36 91
22 31 16 71 51 67 63 89 41 92 36 54 22 40 40 28 66 33 13 80
...
let getArray = function () {
    let rowArrays = sourceData.split('\n');
    let matrixArray = [];
    for (let i = 0; i < rowArrays.length; i++) {
        matrixArray.push(rowArrays[i].split(' '));
    }
    //    Array.from(input, (v, k) => Number.parseInt(v));
    return matrixArray;
}

var getProduct = function (input) {
    var digits = Array.from(input, (v, k) => Number.parseInt(v));
    return digits.reduce((a, b) => a * b, 1);
}

let maxRowSequence = function (input, sliceSize) {
    let maxProduct = 0;
    for (let i = 0; i < input.length; i++) {
        let row = input[i];
        for (let j = 0; j < row.length - sliceSize; j++) {
            let product = getProduct(input.slice(j, j + sliceSize));
            if (product > maxProduct) {
                maxProduct = product;
            }
        }
    }
    console.log('Max product in row: ' + maxProduct);
    return maxProduct;
}
...
let maxColSequence = function (input, sliceSize) {
    ...
    console.log('Max product in column: ' + maxProduct);
    return maxProduct;
}
